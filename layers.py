...
class TaskPainSystem(tf.keras.layers.Layer):
    ...
    def call(self, pred, expected, blended=None, training=False):
        ...
        #     0.1, 0.99)
        exploration = tf.clip_by_value(
            tf.pow(self.per_sample_pain + 1e-3, 0.65) * (1.0 + 0.02 * tf.sin(6.28 * self.per_sample_pain)),
            0.3, 0.99
        )
    ...
    def compute_trait_loss(self, output_logits, expected):
        ...
        entropy_scale = tf.clip_by_value(1.0 - entropy, 0.1, 1.0)  # quanto menor a entropia, maior o impacto
